...
| 关注点 | 无 | `focus_param` + `focus_lines` |
| 会话历史 | 全部保留 | 用户切换关注点后可清空 |
| 行号追踪 | 无 | 树行号 = Mapfile 输出行号 |
...
class TemplateMapper:
    ...
    def get_llm_context_summary(self, object_type: str, focus_lines: str | None = None) -> str:
        """
        为 LLM Prompt 生成该对象类型的参数摘要。

        不是把整个 schema 扔给 LLM，而是生成精简摘要：
        - 只包含 field name + value_type + required + 一句话描述
        - 如果提供了 focus_lines，则标注这些行号
        """
        ...
```
...
class ConfigSession:
    ...
    focus_lines: str | None = None             # 当前关注行号范围
...
class ConfigTree:
    ...
    def rebuild_line_map(self) -> None:
        """按 Mapfile 输出顺序给每个可见节点分配行号。"""
        line = 1
        self.line_map = {}

        def walk(n: TreeNode, indent: int = 0):
            nonlocal line
            self.line_map[n.path] = line
            n.line_start = line
            line += 1

            for leaf in n.leaves():
                self.line_map[leaf.path] = line
                leaf.line = line
                line += 1
        ...
        self.line_map[self.root.path] = line
        self.root.line_start = line
        line += 1
        for child in self.root.nodes():
            walk(child, indent=1)
        self.root.line_end = line
    ...
    def get_focus_lines(self, path: str) -> str | None:
        """计算某对象节点下所有子节点的行号范围。"""
        node = self.get_node(path)
        if isinstance(node, TreeLeaf):
            return f"L{node.line}"
        if isinstance(node, TreeNode):
            return f"L{node.line_start}-L{node.line_end}"
        return None
...
### 4.3 行号 = Mapfile 输出行号（内部维护）

**关键决策**：配置树中每个可见节点分配的行号，**直接对应最终生成的 `.map` 文件中的行号**。

这意味着：

1. 树渲染时按 Mapfile 文本输出顺序分配行号
2. LLM 返回 `param_update` 时可以通过行号定位到具体节点
3. 树结构变化（添加/删除）→ 行号重新计算 → Prompt 中的行号范围同步更新

行号计算由 `ConfigTree.rebuild_line_map()` 完成，不依赖 mappyfile 的 `dumps()`，而是通过 `preview_mapfile()` 预生成文本统一分配（§8.2）。

**前端不显示行号**：行号仅在内部 `line_map` 中维护，用于 LLM 定位。前端界面用扁平路径（如 `LAYER[0].connectiontype`）标识节点位置，更加用户友好。

这样做的好处是：

- 行号计算与 mappyfile 版本解耦
- 可以精确控制哪些节点参与行号分配（如"仅必填"模式不影响行号——所有节点始终占行号）
- 前端无需维护行号显示逻辑，简化渲染

### 4.4 通过行号定位 param_update
...
```json
{
  ...
  "action": "answer",
  "params_update": [
    {"line": 15, "value": "postgis"},
    {"path": "layers.0.name", "value": "buildings"}
  ],
  "question": "已帮你把 L15 的 connectiontype 改为 postgis，同时把图层名设为 buildings。"
}
```
...
class UpdateResolver:
    def __init__(self, line_map: dict[str, int], tolerance: int = 2):
        self.line_map = line_map
        self._reverse = {v: k for k, v in line_map.items()}
    ...
    def resolve(self, update: dict) -> str:
        ...
        if "line" in update:
            line = update["line"]
            path = self._resolve_line(line)
            if path:
                return path
            raise LineNumberNotFound(line)
        raise InvalidUpdate(update)

    def _resolve_line(self, line: int) -> str | None:
        for offset in range(-self.tolerance, self.tolerance + 1):
            if line + offset in self._reverse:
                return self._reverse[line + offset]
        return None
```
...
class DialogueHistory:
    ...
    def set_focus(self, focus_param: str, focus_lines: str | None):
        """
        用户切换关注点时触发。
...
| L3 行号范围 | `focus_param` + `focus_lines` | ~50 tokens |
...
`TemplateMapper.get_llm_context_summary(object_type, focus_lines)` 的生成规则：

```
LAYER 对象参数摘要（当前关注行号 L9-L23）：
- name: string, 必填, 图层名称
...
class ConfigSession:
    ...
    def set_focus(self, path: str | None):
        ...
        self.focus_lines = self.tree.get_focus_lines(path) if self.tree and path else None
        self.history.set_focus(path, self.focus_lines)

    def apply_llm_updates(self, updates: list[dict]) -> None:
        """解析并应用 LLM 返回的更新。"""
        resolver = UpdateResolver(self.tree.line_map)
        for u in updates:
...
class QAService:
    ...
    async def answer(self, session: ConfigSession, question: str) -> QAResult:
        ...
        prompt = self.prompt.render(
            ...
            focus_lines=session.focus_lines,
        ...
        session.tree.rebuild_line_map()
        session.focus_lines = session.tree.get_focus_lines(session.focus_param)
        ...
        return QAResult(
            ...
            line_map=session.tree.line_map,
            focus_param=session.focus_param,
            focus_lines=session.focus_lines,
        )
    ...
    def _build_context_summary(self, session: ConfigSession) -> str:
        ...
        if isinstance(node, TreeNode):
            return self.mapper.get_llm_context_summary(node.object_type, session.focus_lines)
        return ""
...
class ImportService:
    ...
    def import_mapfile(self, session_id: str, content: str,
        ...
        new_session.tree.rebuild_line_map()
...
| `UpdateResolver` | `backend/llm/update_resolver.py` | 行号/路径解析 |
...
### 8.1 树行号 = Mapfile 文件行号（内部维护）

这是 v2 的一个关键约束。行号仅在**后端内部维护**，通过 `preview_mapfile()` 预生成文本统一分配，**前端界面不显示行号**。

实现方法：

1. `ConfigTree.rebuild_line_map()` 调用 `preview_mapfile()` 按 Mapfile 输出格式预生成文本
2. 每输出一行，给对应的树节点分配行号，存入 `line_map: dict[str, int]`
3. 最终导出时复用同一套预生成文本（或重新生成，但保持相同格式）
4. 前端通过 WebSocket 的 `tree_state` / `qa_result` 消息接收 `line_map`，但**不渲染显示**

**`editable=false` 字段的行号处理**：这些字段仍参与行号分配（因为它们在 Mapfile 输出中确实存在），但 `PromptBuilder` 生成 LLM 上下文时会**过滤掉**这些字段，避免 LLM 误建议修改不可编辑的内容。

### 8.2 预生成文本示例

```python
def preview_mapfile(tree: TreeNode) -> str:
    """生成 Mapfile 预览文本，用于分配行号和导出。"""
    lines = []

    def walk(node, indent=0):
        prefix = "  " * indent
        lines.append(f"{prefix}NAME \"{node.get_value('name')}\"")
        for leaf in node.leaves:
            if leaf.descriptor.value_type == "array":
                lines.append(f"{prefix}{leaf.key.upper()}")
                for item in leaf.value:
                    lines.append(f"{prefix}  \"{item}\"")
                lines.append(f"{prefix}END")
            else:
                lines.append(f"{prefix}{leaf.key.upper()} {format_value(leaf.value)}")
        for child in node.children:
            lines.append(f"{prefix}{child.object_type.upper()}")
            walk(child, indent + 1)
            lines.append(f"{prefix}END")
...
LLM 返回的行号可能不完全准确（差 1-2 行）。处理策略：

```python
def resolve_line_with_tolerance(line: int, reverse_map: dict[int, str], tolerance: int = 2):
    """允许 ±tolerance 的行号误差。"""
    for offset in range(-tolerance, tolerance + 1):
        if line + offset in reverse_map:
            return reverse_map[line + offset]
    return None
```
...
直接使用 `mappyfile.dumps()` 从 `ConfigTree` 生成。由于行号已经预计算，导出时复用相同的序列化逻辑：

```python
def export_mapfile(tree: ConfigTree) -> str:
    text = preview_mapfile(tree.root)
    # 可选：再用 mappyfile 解析校验一次
...
后端：ValidationPipeline.validate_field(path, full=False)
  ...
  │
  ▼
后端：ConfigTree.rebuild_line_map()
  │
  ▼
WS: tree_state {
      params_snapshot,
...
后端：session.set_focus(path)
  ...
  └── 更新 session.focus_param / focus_lines
  │
  ▼
WS: focus_state { focus_param, focus_lines }
  │
  ▼
前端：右侧面板显示关注点信息
```
...
前端：用户在输入框输入问题并发送
  │
  ▼
WS: question { text, focus_param?, focus_lines? }
  │
  ▼
...
WS: tree_state { params_snapshot, validation_state, errors, line_map, can_export }
  │
  ▼
...
interface TreeState {
  ...
  validation_errors: Array<{ path: string; message: string; line?: number }>;
  can_export: boolean;
  focus_param?: string;
  focus_lines?: string;
...
| LLM 行号无法解析 | 静默忽略该行更新，记录日志，继续处理其他更新；不影响用户直接编辑 |
...
| 行号与 LLM 引用的不一致 | `rebuild_line_map()` 未及时调用 | 任何树变更后都要调用 |

### 16.2 日志级别

- `INFO`：用户操作（更新参数、切换关注点、提问、导出）
- `DEBUG`：Prompt 内容、LLM 原始响应、行号映射
